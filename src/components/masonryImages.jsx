...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    return items.map(child => {
      ...
      const x = (col * (columnWidth + gap)) + padding;
      
      // Mobile view ke liye fixed height 500px
      const isMobile = width < 768; // 768px se chhota screen mobile consider karenge
      const height = isMobile ? 250 : child.height / 2; // Mobile: 500px, Desktop: original height
      
      const y = colHeights[col];

      colHeights[col] += height + gap;
      return { ...child, x, y, w: columnWidth, h: height, isMobile };
    });
  }, [columns, items, width, padding]);
  ...
  const containerHeight = useMemo(() => {
    ...
    return maxHeight + padding; // Add bottom padding
  }, [grid, padding]);
        ...
        >
          <div
            ...
            style={{ 
              ...
              height: `${item.h}px` // Explicit height set karo
            }}
          >
            {colorShiftOnHover && (
